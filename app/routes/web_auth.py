...
def _upsert_account_for_contact(contact: str) -> Optional[str]:
    ...
    IMPORTANT:
    - DO NOT manually generate UUID.
    - Let Postgres default generate UUID.
    """
    ...
    try:
        ...
        # Insert WITHOUT account_id (let DB generate UUID)
        insert_res = (
            _sb()
            .table("accounts")
            .insert({
...
def verify_otp():
    ...
    row = rows[0]
    if _now_utc() > datetime.fromisoformat(row["expires_at"]):
