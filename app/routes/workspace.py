...
from app.services.accounts_service import get_account_by_id, require_auth
...
def get_workspace(workspace_id: str):
    ...
    try:
        ...
        logging.error(f"Failed to get workspace: {e}")
        return jsonify({"ok": False, "error": str(e)}), 500
...
def create_workspace():
    ...
    try:
        ...
        if not workspace_result.data:
            return jsonify({"ok": False, "error": "Failed to create workspace"}), 500
...
def get_workspace_members(workspace_id: str):
    ...
    try:
        ...
        logging.error(f"Failed to get workspace members: {e}")
        return jsonify({"ok": False, "error": str(e)}), 500
