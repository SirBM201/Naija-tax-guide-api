...
from app.services.web_quiz_service import (
    ...
    get_quiz_score,
    submit_quiz_answer,
    unlock_quiz_detailed_explanation,
)
...
def web_quiz_detailed_explanation():
    ...
    body: Dict[str, Any] = request.get_json(silent=True) or {}
    res = unlock_quiz_detailed_explanation(account_id=account_id, body=body)
    if res.get('ok'):
        return jsonify(res), 200

    error = _safe_text(res.get('error') or res.get('reason') or 'q5_unavailable')
    http_status = 404 if error == 'question_not_found' else 400 if error == 'explanation_not_available' else 402
